...
public class ModuleButton {
	...
	public ModuleButton(Module imod, Panel pl) {
		 ...
		 */
		if (Dawn.instance.settingsManager.getSettingsByMod(imod) != null)
			for (Setting s : Dawn.instance.settingsManager.getSettingsByMod(imod)) {
				if (s.isCheck()) {
	...
	public boolean mouseClicked(int mouseX, int mouseY, int mouseButton) {
		...
		if (mouseButton == 0) {
			...
			//if(Dawn.instance.settingsManager.getSettingByName("Sound").getValBoolean())
			...
			if (menuelements != null && menuelements.size() > 0) {
				...
				Dawn.instance.clickGui.closeAllSettings();
				this.extended = b;
